...
const NewsLetter = () => {
    ...
    const handleSubscribe = () => {
        if (email && email.includes('@')) {
            alert(`Thanks for subscribing! Confirmation sent to ${email}`);
            setEmail('');
        } else {
    ...
    return (
        <div className='flex flex-col items-center max-w-5xl lg:w-full rounded-2xl px-4 py-12 md:py-16 mx-2 lg:mx-auto my-30 bg-gray-900 text-white'>
            ...
            <div className='flex flex-col md:flex-row items-center justify-center gap-4 mt-6'>
                <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className='bg-white/10 px-4 py-2.5 border border-white/20 rounded outline-none max-w-66 w-full' placeholder='Enter your email' />
                <button onClick={handleSubscribe} className='flex items-center justify-center gap-2 group bg-black px-4 md:px-7 py-2.5 rounded active:scale-95 transition-all hover:bg-gray-800 cursor-pointer'>
                    Subscribe
